...
$X_t$ número de motos vendidas en $t$ semanas eres un proceso poisson $Po(\lambda \cdot t=1\cdot=t)$. Así que $X_t$ es una $Po(t)$.
...
**Apartado 3.** La variable $T$ tiempo entre dos ventas consecutivas  de  este proceso poisson es una v.a. $Exp(\lambda=1)$. Ahora  nos piden $P(X>2)=1-P(X\geq 2)=1-\left(1-e^{-1\cdot 2}\right)=`r 1-(1-exp(-2))`.$

Con R

```{r}
1-pexp(2,rate=1)
pexp(2,rate=1,lower.tail=FALSE)
```

O también es $P(X_2=0)=\frac{2^0}{0!}\cdot \exp{-2}=`r exp(-2)`.$
