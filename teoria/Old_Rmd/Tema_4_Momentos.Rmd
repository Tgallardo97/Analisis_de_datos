---
title: "Tema 4 - Variables Aleatorias. Complementos (actualizar desde la trans 35)"
...
library(reticulate)
library(latex2exp)
## CORREGIDO HAST MÉTODOS DE TRASFORMACION 35
```
...
Sea $X$ una variable aleatoria. Definimos el \blue{momento de orden $n$} como 
...
* Los momentos de orden $n$ caracterizan una variable $X$. Si conocemos todos los momentos de orden $n$, podemos deducir cuál es la distribución de $X$.
...
\blue{Momentos de orden $n$ de una variable de Bernoulli} de parámetro $p$
...
\blue{Momento de orden $n$ de una variable normal de parámetros $\momento=0$ y $\sigma =1$}

Recordemos que su función de densidad es: $f_X(x)=\frac{1}{\sqrt{2\pi}}\mathrm{e}^{-\frac{x^2}{2}},$ para $x\in \mathbb{R}$.

Su momento de orden 1 será la esperanza de $X$: $\momento_1 = 0$ y su momento de orden 2 es: 
...
\blue{Ejemplo:} Los momentos de orden impar $n$ son  cero ya que integramos una función impar centrada en $0$
$$\momento_n = E\left(X^n\right)=\int_{-\infty}^\infty \frac{1}{\sqrt{2\pi}}\mathrm{e}^{-\frac{x^2}{2}}\cdot x^n\, dx = 0.$$


Es decir, si consideramos $g(x)=\frac{1}{\sqrt{2\pi}}\mathrm{e}^{-\frac{x^2}{2}}\cdot x^n$, se verifica $g(-x)=-g(x)$, para todo $x\in\mathbb{R}$.
...
Sea $X$ una variable aleatoria. Definimos el \blue{momento central de orden $n$} como 
...
Los momentos centrales de orden $n$ también tienden a infinito a medida que $n$ crece: $\lim\limits_{n\to\infty}\momentocentral_n = \lim\limits_{n\to\infty}\frac{a_n}{\lambda^n}=\infty$: 
$$
...
Recordemos que su función de densidad es: $$f_X(x)=\frac{1}{\sqrt{2\pi}\sigma}\mathrm{e}^{-\frac{(x-\mu)^2}{2\sigma^2}},\mbox{ para }  x\in \mathbb{R}.$$

Su momento central de orden 2 será la varianza $\sigma^2$: $\momentocentral_2 =\sigma^2.$

Los momentos centrales de orden $n$ impar  son cero ya que integramos una función impar respecto $x=\mu$:
...
Es decir, la función  
$g(x)=\frac{1}{\sqrt{2\pi}\sigma}\mathrm{e}^{-\frac{(x-\mu)^2}{2\sigma^2}}\cdot (x-\mu)^n$, cumple que $g(\mu-x)=-g(\mu +x)$, para todo $x\in\mathbb{R}$.
...
\blue{Definición:} Una variable aleatoria tiene \blue{asimetría positiva} si su función de densidad o de probabilidad presenta una cola a la 
**derecha** y \blue{asimetría negativa} si su función de densidad o de probabilidad presenta cola a la **izquierda**.
...
```{r,echo=FALSE,fig=TRUE,warning=FALSE,fig.align='center'}
par(mfrow=c(1,2))
x=seq(from=0,to=21, by=1)
plot(x,dbinom(x,20,0.9),type="l",ylab="función de probabilidad",col="red")
x=seq(from=0,to=100, by=0.1)
plot(x,dchisq(x,10),type="l",ylab="función de densidad",col="red")
par(mfrow=c(1,1))
```


## Coeficiente  asimetría de una variable aleatoria

\blue{Definción:} La \blue{asimetría de una variable aleatoria} $X$ se calcula a partir de sus momentos centrales de segundo y tercer orden:
$$
...
\red{Propiedad:} Usando la relación entre los momentos centrales y los momentos, podemos expresar el **coeficiente de asimetría** en función de los momentos:
...
## Gráfica asimetrtía Bernoulli

```{r,echo=FALSE,warning=FALSE,fig=TRUE,fig.height=4,fig.align='center'}
par(mfrow=c(1,2))
plot(c(-0.1,1.1,-0,1.1),c(-0.1,-0.1,1,1),
     type="n",xlab="",ylab="",xaxt="n",
     yaxt="n",axes=FALSE,xlim=c(-0.5,1.5))
...
plot(x,dexp(x,rate=2),type="l",ylab="función de densidad",col="red")
```
...
## Gráfico asimetría distribución $N(\mu=1.5, \sigma=2)$

```{r,echo=FALSE,warning=FALSE,fig=TRUE,fig.align='center'}
mu=1.5
sigma=2
x=seq(from=mu-5,to=mu+5,by=0.01)
plot(x,dnorm(x,mu,sigma),type="l",ylab="función de densidad",
     col="blue",xlim=c(mu-5,mu+5))
text(mu+0.5,0.14,TeX("$x=\\mu=1.5$"))
abline(v=mu,col="red")
```
...
## Ejemplo de cálculo de curtosis $Ber(p)$
...
Como acabamos de ver el cálculo de los **momentos** o los **momentos centrados** de una variable aleatoria $X$ puede ser muy complicado y muy tedioso.
...
Definimos la \blue{función generatriz de momentos $\FunGenMom_X(t)$} en el valor $t$ como: $\FunGenMom_X(t)=E\left(\mathrm{e}^{tX}\right).$
...
$$
Es decir  el momento de orden $n$ de $X$  se puede obtener como la derivada $n$-ésima de la función generatriz de momentos evaluada en $t=0$.
